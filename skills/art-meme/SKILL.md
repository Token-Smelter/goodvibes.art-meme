...
**BLUF:** Distill the moment into an emotional structure, filter the corpus with `scripts/match.py` (hard gates in code), pick the funniest fit, write ≤5-word labels mapping session entities to the painting's figures, render with `scripts/render.py`, and show the image. If nothing fits, say so — never force a pick.
